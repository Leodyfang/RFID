 ...
 * 定义
 ******************************************************************************/
#define MAX_LEN 16   // 数组最大长度,根据Mifare 1单个数据块长度16字节

//MF522命令字PCD,写入MFRC522命令寄存器(CommandReg 0x01)
...
//Mifare_One卡片命令字PICC，命令Mifare 1完成相应操作
...
#define PICC_DECREMENT        0xC0               //值增
#define PICC_INCREMENT        0xC1               //值减
...
class RFID
{
  public:
	...
	void init();
	void reset();
	inline void setBitMask(unsigned char reg, unsigned char mask);
	inline void clearBitMask(unsigned char reg, unsigned char mask);
	void antennaOn(void);
	...
	inline unsigned char readMFRC522(unsigned char addr);
